...
func crypterFor(t Transform) (cryptoutil.ESPCrypter, error) {
	if t.EncrID != 12 { // IANA ENCR_AES_CBC
		return nil, fmt.Errorf("esp: unsupported encryption transform %d", t.EncrID)
	}
	var integrity *cryptoutil.Integrity
	switch t.IntegID {
	case 2: // IANA AUTH_HMAC_SHA1_96
		integrity = cryptoutil.NewHMACIntegrity(sha1.New, 20, 12)
	case 12: // IANA AUTH_HMAC_SHA2_256_128
		integrity = cryptoutil.NewHMACIntegrity(sha256.New, 32, 16)
	default:
		return nil, fmt.Errorf("esp: unsupported integrity transform %d", t.IntegID)
	}
	return cryptoutil.NewAESCBCESPCrypter(int(t.EncrKeyLn), t.EncKey, integrity, t.IntegKey)
}
...
// inside it. Callers decide when and how far to pad.
//
