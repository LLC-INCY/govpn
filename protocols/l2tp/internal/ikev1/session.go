...
type Result struct {
	...
	// UDP-encapsulated there. govpn always negotiates this.
	NATT bool
...
func (s *Session) dispatch(h header, first uint8, rest []byte) error {
	...
	case exchangeInformational:
		return s.handleInformational(h, first, rest)
	...
	if h.flags&flagEncryption == 0 {
		if payloads, _, err := parsePayloads(first, rest); err == nil {
			...
			// A responder retransmits MM4 when it did not accept MM5. MM4 is
			// plaintext, while the MM6 this state expects is encrypted; treating
			// the duplicate as MM6 produces a misleading CBC payload-length error.
			if s.cfg.Role == Initiator && s.state == stWaitMM6 {
				_, hasKE := findPayload(payloads, payloadKE)
				_, hasNonce := findPayload(payloads, payloadNonce)
				if hasKE && hasNonce && s.lastSent != nil {
					s.logger.Printf("ikev1: received duplicate MM4; retransmitting MM5")
					return s.cfg.Send(s.lastSent, s.lastSentNATT)
				}
			}
		}
	}
